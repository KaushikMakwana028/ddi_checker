...
<div class="form-page-container">
        ...
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <div class="page-eyebrow">
                    <i class="bi bi-shield-plus"></i> Clinical Interaction Engine
                </div>
                <h2 class="page-title">Add Drug Interaction Rule</h2>
                <p class="page-subtitle">Configure clinical severity and contraindication remarks for a pair of medicines.</p>
            </div>
        </div>
    </div>
    ...
    <div class="card border-0 rounded-4 shadow-sm form-card">
        <div class="card-header bg-white border-bottom py-3.5 px-4 d-flex align-items-center gap-3">
            ...
            <div>
                <h5 class="mb-0 fw-bold text-dark">Interaction Rule Model</h5>
                <small class="text-muted">Drug pairs are automatically normalized (order-independent) in the clinical engine.</small>
            </div>
        </div>

        <div class="card-body p-4 p-md-5">
            <?php echo form_open('admin/interactions/add', ['id' => 'addRuleForm', 'autocomplete' => 'off']); ?>
                <div class="row g-4 mb-4">
                            ...
                            <select id="drug_a_id" name="drug_a_id" required>
                                ...
                                <?php foreach ($drugs as $d): ?>
                                    ...
                                    <option value="<?php echo $d['id']; ?>"
                                            ...
                                            <?php echo set_select('drug_a_id', $d['id'], ($drug_a_id == $d['id'])); ?>>
                            ...
                            <select id="drug_b_id" name="drug_b_id" required>
                                ...
                                <?php foreach ($drugs as $d): ?>
                                    ...
                                    <option value="<?php echo $d['id']; ?>"
                                            ...
                                            class="<?php echo $isOutOfStock ? 'opt-out-of-stock' : ''; ?>"
                                            style="<?php echo $isOutOfStock ? 'color: #dc2626; font-weight: 600;' : ''; ?>"
                                            <?php echo set_select('drug_b_id', $d['id'], ($drug_b_id == $d['id'])); ?>>
                                        <?php echo html_escape($d['drug_name']) . $cat; ?> — <?php echo $isOutOfStock ? '[Out of Stock: 0' . $unit . ']' : 'Qty: ' . number_format($qty) . $unit; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-field">
                            ...
                            <div class="input-icon">
                                ...
                                <input type="text" id="source" name="source" value="<?php echo set_value('source'); ?>" placeholder="e.g. FDA Label, Stockley's Drug Interactions, BNF 84">
                            </div>
                    ...
                    <div class="col-12">
                        <div class="form-field">
                            ...
                            <textarea id="remarks" name="remarks" rows="4" required placeholder="Describe the adverse mechanism, risk of toxicity, clinical symptoms to monitor, and recommended corrective actions or alternative therapies..."><?php echo set_value('remarks'); ?></textarea>
                            <span class="field-hint">Detailed pharmacological notes presented to practitioners during prescription evaluation.</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-actions mt-5 pt-3 border-top d-flex align-items-center justify-content-between flex-wrap gap-3">
                    ...
                    <button type="submit" class="btn-page-primary">
                        <i class="bi bi-check-lg"></i> <span>Save &amp; Register Rule</span>
                    </button>
                </div>
            <?php echo form_close(); ?>
        </div>
